...
bool rayOnSegment(P &a, P &b, P &p) {
    ...
    if (a.Y > b.Y) return rayOnSegment(b, a, p);
...
int main() {
    ...
    size_t kVertices, kPoints; std::cin >> kVertices >> kPoints;
    ...
    for (size_t i = 0; i < kPoints; i++) {
        ...
        for (size_t j = 0; j < kVertices; j++) {
            ...
            if (isOnLine(vertices_list[next_index], vertices_list[j], cur_point)) {
                ...
                break;
            } else if (rayOnSegment(vertices_list[next_index], vertices_list[j], cur_point)) {
                intersections++;
            }
        }
