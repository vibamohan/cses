...
public:
    ...
    bool union_sets(int a, int b) {
        ...
        if (rank[find_a] == rank[find_b]) rank[find_a]++;
        return true;
    }
...
void dfs(int start, int prev) {
    // I think we could also do this in bfs, but its more logical in dfs and i like this way more
    if (cycled) return;
    visited[start] = true;
    parent[start] = prev;
    for (int neighbour : adj[start]) {
        ...
        if (visited[neighbour]) {
            ...
            // keep traversing till we meet neighbour again (avoid infinite cycle traversing)
            while (cur != neighbour) {
                path.push_back(cur);
                cur = parent[cur];
            }
